    ...
    <el-card style="margin-top:16px">
      ...
      <el-table :data="employeeList" border stripe v-loading="loadingEmployees">
        ...
        <el-table-column label="所属工区" min-width="150">
          <template #default="{ row }">{{ row.areaName || userStore.userInfo?.orgName || '-' }}</template>
        </el-table-column>
      ...
      <el-empty v-if="!loadingEmployees && employeeList.length === 0" description="该工区暂无人员" />
...
import type { WorkReport, MonthlyPeriod, Employee, WorkItem } from '@/types'
